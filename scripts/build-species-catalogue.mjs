...
async function enrich() {
  while (cursor < taxa.length) {
    ...
    let common = taxon.scientificName;
    try {
      const response = await fetch(`https://www.marinespecies.org/rest/AphiaVernacularsByAphiaID/${taxon.taxonID}`);
      if (response.ok) {
        const names = await response.json();
        const english = names.find(name => name.language_code === "eng" && name.vernacular);
        if (english) common = english.vernacular.replace(/^./, letter => letter.toUpperCase());
      }
    } catch {}
    records[index] = { common, scientific:taxon.scientificName, detail:taxon.family || taxon.class || taxon.phylum || "Marine species", category:group, icon, color, aphiaID:taxon.taxonID };
    completed++;
    if (completed % 50 === 0) console.log(`Resolved ${completed}/${taxa.length} vernacular names`);
  }
}
await Promise.all(Array.from({ length:12 }, enrich));
